...
async def test_event_bus_publish_subscribe():
    ...
    event = Event(
        event_type=EventType.SIMULATION_STARTED, source="test", data={"simulation_id": "test-001"}
    )
...
async def test_event_bus_multiple_subscribers():
    ...
    event = Event(event_type=EventType.WORLD_CREATED, source="test", data={"world_id": "world-001"})
...
async def test_event_bus_unsubscribe():
    ...
    event1 = Event(
        event_type=EventType.CHARACTER_CREATED, source="test", data={"character_id": "char-001"}
    )
    ...
    event2 = Event(
        event_type=EventType.CHARACTER_CREATED, source="test", data={"character_id": "char-002"}
    )
...
async def test_event_bus_history():
    ...
    # 发布多个事件
    for i in range(5):
        event = Event(event_type=EventType.SIMULATION_STARTED, source="test", data={"index": i})
        await bus.publish(event)
