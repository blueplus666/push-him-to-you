...
class SQLiteStateStore(StateStore):
    ...
    async def get_simulation(self, simulation_id: str) -> Optional[SimulationContext]:
        ...
        def _get():
            conn = self._get_connection()
            cursor = conn.cursor()
            cursor.execute('SELECT * FROM simulation_contexts WHERE simulation_id = ?', (simulation_id,))
            row = cursor.fetchone()
            result = None
            if row:
    ...
    async def update_simulation(self, simulation_id: str, updates: Dict[str, Any]) -> bool:
        ...
        def _update():
            ...
            query = f"UPDATE simulation_contexts SET {', '.join(set_clauses)} WHERE simulation_id = ?"
            cursor.execute(query, values)
            conn.commit()
            result = cursor.rowcount > 0
            self._close_connection(conn)
    ...
    async def delete_simulation(self, simulation_id: str) -> bool:
        ...
        def _delete():
            conn = self._get_connection()
            cursor = conn.cursor()
            cursor.execute('DELETE FROM simulation_contexts WHERE simulation_id = ?', (simulation_id,))
            conn.commit()
            result = cursor.rowcount > 0
            self._close_connection(conn)
            return result
